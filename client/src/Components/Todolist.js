...
function Todolist(props) {
  const todolist = props.todolist.map((task, index) => {
    const taskcomplete = (task) => {
      axios.put(`https://mern-todo-server-ruddy.vercel.app/tasks/${task._id}`, {
        _id: task._id,
        todo: task.todo,
    ...
    const removeTask = (id) => {
      axios.delete(`https://mern-todo-server-ruddy.vercel.app/tasks/${id}`)
    ...
    return (
      <li key={index}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <label className="container" style={{ marginRight: '7px' }}>
            <input 
              type="checkbox" 
            ...
            />
            <div 
              className={task.isComplete ? 'checkmark' : 'checkbox'} 
